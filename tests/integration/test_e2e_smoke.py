...
class TestE2ESmoke:
    ...
    async def test_full_pipeline_three_apps(
        self, db_conn, sample_session, log_repo, ev_repo, verif_repo, app_repo, three_app_csv, tmp_path
    ):
        ...
        def make_doc_parser():
            ...
            async def _run(state):
                return {**state,
                    "chunks": [{"content": MOCK_FIRECRAWL_CONTENT,
                                "source_url": state.get("documentation_url",""),
                                "chunk_index": 0, "token_count": 300}],
                    "pages_parsed": 1, "parse_method": "firecrawl",
        ...
        def make_auth_extractor():
            ...
            async def _run(state):
                return {**state,
                    "auth_result": [{"method":"API Key","confidence":0.97,
                                     "evidence_snippet":"Bearer token","source_url":state.get("documentation_url",""),
                                     "oauth_flows":[]}],
        ...
        def make_api_analyzer():
            ...
            async def _run(state):
                return {**state,
                    ...
                    "rate_limits": "100/sec",
                    "openapi_url": None, "graphql_schema_url": None,
                    "api_confidence": 0.94,
        ...
        for _ in range(3):
            ...
            assert final_state.get("buildability_verdict") in (
                "Fully Buildable", "Buildable with Workarounds", "Blocked"
            )
